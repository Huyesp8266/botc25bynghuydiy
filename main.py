...
# ==================== 1. FLASK WEB SERVER (KHÔNG TRÙNG/TẠO MỚI KHI RELOAD) ====================

app = Flask(__name__)
# Đặt Secret Key cố định để lưu phiên truy cập (session)
...
def get_key_site():
    # Kiểm tra người dùng đã có Key trong Session hiện tại chưa
    if 'current_key' in session:
        user_key = session['current_key']
        # Nếu key đã được nhập rồi thì tạo key mới cho lượt tiếp theo
        ...
        # Nếu chưa nhập thì hiển thị lại Key cũ (Reload không đổi)
        return render_template_string(HTML_TEMPLATE, key=user_key)
    
    # Tạo Key mới cho lượt đầu tiên
    new_key = generate_random_key()
    KEYS_DATABASE[new_key] = {"used": False, "created_at": time.time()}
    session['current_key'] = new_key
    return render_template_string(HTML_TEMPLATE, key=new_key)
